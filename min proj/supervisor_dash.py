...
from tkinter import ttk, messagebox, Label, Frame, Button, Tk, LEFT, Toplevel, StringVar, BooleanVar, Checkbutton, LabelFrame, END, BOTH
from PIL import Image, ImageTk
import os
import tkinter.font as tkfont
from face_img import facemain, mainimg
from finalchat import mainchat
from supervisor_case import Forensyncapp
...
class AdminDashboard:
    ...
    def view_logs_window(self):
        window = Toplevel(self.root)
        window.title("Activity Logs")
        window.geometry("1000x600")
        window.configure(bg=self.bg_color)
        
        Label(window, text="Activity Logs", font=("Segoe UI", 20, "bold"), 
              bg=self.bg_color, fg=self.text_color).pack(pady=20)
        ...
        Button(window, text="Refresh", command=refresh_logs,
               bg=self.primary_color, fg=self.text_color,
               font=("Segoe UI", 11)).pack(pady=20)
        
        refresh_logs()  # Load logs when window opens

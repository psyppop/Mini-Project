        ...
        try:
            ...
            if login_type == "Investigator":
                ...
                if user:
                    ...
                    cursor.execute("SELECT * FROM cases WHERE investigator_id = %s", (user[0],)) 
                    cases = cursor.fetchall()
                    dashboard = ForenSyncDashboard(app.root, username)  # Pass role
                    dashboard.show_dashboard(username, cases)
                else:
                    messagebox.showerror("Login Error", "Invalid Username or Password.")

            elif login_type == "Supervisor":
                cursor.execute(
                ...
                if user:
                    ...
                    dashboard = ForenSyncDashboard(app.root, username, "supervisor")  # Pass role
...
# Assuming these functions exist to handle different dashboards

...
# Function to edit a selected case
 ...
 def delete_case(self, index):  # Add 'index' as a parameter
    case_title = self.cases[index]  # Use self.cases to access the list of cases
    if messagebox.askyesno("Delete", "Are you sure you want to delete this case?"):
        ...
        self.display_cases()  # Use self.display_cases to refresh the list

 def open_case_options(self, case_title):  # Add 'case_title' as a parameter
    self.listbox_frame.place_forget()  # Hide the original list frame
    self.create_options_frame(case_title)

# Function to create and display the options frame
 def create_options_frame(self, case_title):  # Add 'case_title' as a parameter
    ...
    " Report": {
        ...
        "command": self.open_report_frame  # Direct command for Report
    },
    "Evidence": {
        "x": 300, "y": 80, "width": 160, "height": 120,
        "icon": self.view_icon, "compound": "top",
        "command": self.open_evidence_frame  # Direct command for Evidence
    },
    }
 ...
 def open_report_frame(self):  # Add 'self' as a parameter
 ...
 def open_edit_form(selected_index):
    ...
    try:
        ...
        img.thumbnail((150, 150))  # Resize for previewCX
        photo = ImageTk.PhotoImage(img)
...
# Function to search for cases
 ...
 def display_cases(self, case_list=None):
        # Clear existing widgets in the case_frame_container
        ...
        for index, case in enumerate(case_list):
            ...
            anchor='w', 
            width=61, 
            command=lambda case=case: self.open_case_options(case)  # Pass 'case' to open_case_options
            )
            ...
            command=lambda idx=index: self.delete_case(idx),  # Pass 'idx' to delete_case
            bg='#1b6c7e', 
            borderwidth=0
            )
 ...
 def fetch_cases(self):
        ...
        self.cases = [case[0] for case in self.db_cursor.fetchall()]  # Fetch all case titles
        self.display_cases()  
 ...
 def main(self):
        ...
        self.add_button.place(x=10, y=40)  # Positioned within the listbox frame, below the title frame

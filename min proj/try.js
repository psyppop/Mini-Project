...
// New endpoint specifically for view-only mode
app.get('/view-report/:firNumber', (req, res) => {
    ...
    db.query(sql, [firNumber], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            // Add view-only flag to the response
            report.viewOnly = true;
...
// Keep existing endpoints but add view-only checks
app.post('/save-report', (req, res) => {
    // Check for view-only mode
    if (req.body.viewOnly) {
        return res.status(403).json({ error: 'This report is in view-only mode and cannot be modified.' });
    }

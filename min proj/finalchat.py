...
class Investigator2ChatClient:
    ...
    def load_chat_history_from_db(self, contact_username):
        ...
        messages = ref.get() or {}
        return sorted(messages.values(), key=lambda x: x['timestamp'])
    ...
    def send_private_message(self):
        ...
        if message and self.current_chat:
            ...
            self.client_socket.emit('private_message', {'recipient': self.current_chat, 'message': message})
    ...
    def display_message(self, sender, message):
        ...
        if not is_own_message:
            Label(inner_frame, text=sender, font=("Segoe UI", 9, "bold"), bg="#1e1e2f", fg="#5DADE2").pack(anchor=W)

        message_label = Label(inner_frame, text=message, font=("Segoe UI", 11), wraplength=400,
                              justify=LEFT, bg=bg_color, fg="white", padx=12, pady=8, bd=0)
        message_label.pack(anchor=E if is_own_message else W)

        self.canvas.yview_moveto(1.0)

def mainchat(chat_window=None, dashboard_root=None):
    ...
    if login_result:
        ...
        # Use the provided chat window or create a new one
        if chat_window is None:
            chat_window = Tk()
        app = Investigator2ChatClient(chat_window, username, user_id)
